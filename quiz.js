...
function Quiz(player, question_array, number_tries) {
	this.player 		= player
	this.question_array = question_array
	this.score 			= 0
	this.question_index = 0
	this.number_tries = number_tries
	...
	this.checkAnswer = function(err, answer) {
    	if (answer.toLowerCase() === question_array[this.question_index].answer.toLowerCase()) {
			...
			if (this.question_index < this.question_array.length) {
			this.post_questions();
			} else {
			...
			if (question_array[this.question_index].counter >= this.number_tries) {
				console.log("Uh oh! You have exceeded the number of tries!... Donald? Is it you?")
			} else {
				question_array[this.question_index].counter++
				this.post_questions();
			}
		};
	};
	...
	this.post_questions = function() {
		this.read_score();
		console.log("You have " + (this.number_tries - question_array[this.question_index].counter) + " tries left...")
...
var quiz = new Quiz("Alvaro", question_arr, 3);
